...
![Horse populations for all provinces in Canada from 1906 - 1972.](../results/horse_pops_plot.png){#fig-horse-pop-plot width=80%}

We can see from @fig-horse-pop-plot that Ontario, Saskatchewan and Alberta 
...
We present the standard deviations in @tbl-std-dev-horse.

```{r}
#| label: tbl-std-dev-horse
...
Additionally, note that in @tbl-std-dev-horse we consider the sample standard deviation of the number of horses during the same time span as @fig-horse-pop-plot.

![Horse populations for the province with the largest standard deviation.](../results/horse_pop_plot_largest_sd.png){#fig-horse-pop-plot-largest width=60%}

```{r}
province <- horses_sd_table[1] |> head(1)
```

In @fig-horse-pop-plot-largest we zoom in and look at the province of `{r} province`, which had the largest spread of values in terms of standard deviation.